...
return {
  ...
  args = {
    ...
    function(ctx)
      if vim.bo[ctx.bufnr].filetype == "cpp" then
        return "--language=c++"
      else
        return "--language=c"
      end
    ...
    "--quiet",
    function(ctx)
      if vim.fn.isdirectory(ctx.cwd .. "/build") == 1 then
  ...
  stream = "stderr",
  parser = require("splint.parser").from_pattern(pattern, groups, severity_map, { ["source"] = "cppcheck" }),
}
